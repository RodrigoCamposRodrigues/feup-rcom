...
int packetRecognition(unsigned char *received_packet, int received_packet_size, FILE *new_file, unsigned char sequence_number_checker)
{    
    ...
    switch(linkLayer.role)
    {
        ...
        case LlRx:{               
            ...
            while(1){
                ...
                if(packetRecognition(received_packet, received_packet_size, new_file, sequence_number_checker) == FALSE)
                    break;
                if(received_packet[0] == 1){
                    printf("Changed sequence number\n");
